...
export default function Payloan() {// this fn gets all the requested obj by other to this user.
   const history=useHistory();
    /*const retrieve=async(user)=>{// gets the acc of that user
     ...
     /* const r=async(acc)=>{
        const response=await(api2.get("/requestmoney"));// gets all the requested objects
        const re=[]
        // console.log(response.data);
        // const sol=response.data;
        const sol=response.data;
            for (let index = 0; index < sol.length; index++) {
                ...
                if(sol[index].to === acc)// filter object , requested by others
                {
                    re.push(sol[index]);
                }
